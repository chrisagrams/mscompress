...
describe("MSZX", () => {
  ...
  describe("MSZXFile from test fixture", () => {
    let mszxFile: MSZXFile;
    ...
    it("should open the test.mszx fixture", () => {
      mszxFile = MSZXFile.open(MSZX_PATH);
      expect(mszxFile).toBeDefined();
      expect(mszxFile.archive_path).toBe(MSZX_PATH);
    });

    it("should read manifest with correct metadata", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);
      ...
      expect(manifest.spectra_file).toBe("temp.msz");
    });

    it("should have 100 spectra", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      expect(mszxFile.spectra.length).toBe(100);
    });

    it("should access spectra by index", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const first = mszxFile.spectra.get(0);
      expect(first).toBeDefined();
      expect(first.scan).toBeGreaterThan(0);
      expect(first.msLevel).toBeGreaterThanOrEqual(1);

      const last = mszxFile.spectra.get(99);
      expect(last).toBeDefined();
      expect(last.scan).toBeGreaterThan(0);
    });

    it("should iterate over spectra", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      let count = 0;
      for (const spectrum of mszxFile.spectra) {
        expect(spectrum.scan).toBeGreaterThan(0);
        count++;
        if (count >= 5) break;
      }
      expect(count).toBe(5);
    ...
    it("should get m/z binary data", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const mz = mszxFile.getMzBinary(0);
      expect(mz).toBeDefined();
      expect(mz.length).toBeGreaterThan(0);
      expect(mz[0]).toBeGreaterThan(0);

      // m/z values should be sorted (ascending)
      for (let i = 1; i < mz.length; i++) {
        expect(mz[i]).toBeGreaterThanOrEqual(mz[i - 1]);
      }
    });

    it("should get intensity binary data", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);
      ...
      for (let i = 0; i < inten.length; i++) {
        expect(inten[i]).toBeGreaterThanOrEqual(0);
      }
    });

    it("should get XML for a spectrum", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const xml = mszxFile.getXml(0);
      expect(xml).toBeDefined();
      expect(xml.length).toBeGreaterThan(0);
      expect(xml).toContain("<spectrum");
    });

    it("should have annotation entries", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const annotations = mszxFile.annotation_files;
      expect(annotations.length).toBe(1);
      expect(annotations[0].filename).toBe("HSA.pepXML.zst");
      expect(annotations[0].format).toBe("pepxml");
      expect(annotations[0].compressed).toBe(true);
      expect(annotations[0].num_records).toBe(100);
    });

    it("should read and decompress annotation file", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const content = mszxFile.getAnnotationFile("HSA.pepXML.zst");
      expect(content).toBeDefined();
      expect(content.length).toBeGreaterThan(0);

      // Should be decompressed pepXML content
      const text = content.toString("utf-8");
      expect(text).toContain("<?xml");
      expect(text).toContain("pepXML");
    });

    it("should get annotation files by format", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const pepxml = mszxFile.getAnnotationFilesByFormat("pepxml");
      expect(pepxml).toEqual(["HSA.pepXML.zst"]);

      const tsv = mszxFile.getAnnotationFilesByFormat("percolator_tsv");
      expect(tsv).toEqual([]);
    });
    ...
    it("should provide describe() with archive info", () => {
      ...
      const desc = mszxFile.describe();
      expect(desc.archive).toBeDefined();
      expect((desc.archive as Record<string, unknown>).path).toBe(MSZX_PATH);
    });

    it("should decompress to mzML", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mszx-decompress-test-"));
      const outputPath = path.join(tmpDir, "output.mzML");

      try {
        const result = mszxFile.decompress(outputPath);
    ...
    it("should throw RangeError for out-of-bounds index", () => {
      ...
      expect(() => mszxFile.getMzBinary(-1)).toThrow(RangeError);
      expect(() => mszxFile.getMzBinary(100)).toThrow(RangeError);
      expect(() => mszxFile.getIntenBinary(-1)).toThrow(RangeError);
      expect(() => mszxFile.getXml(100)).toThrow(RangeError);
    });

    it("should throw for non-existent annotation file", () => {
      if (!mszxFile) mszxFile = MSZXFile.open(MSZX_PATH);

      expect(() => mszxFile.getAnnotationFile("nonexistent.tsv")).toThrow(
