"""Inverse-path machinery: mzML / .msz / .mszx -> parquet.

Public surface:
    `to_parquet(input, output_path, ...)` — polymorphic over input file type
    via the shared `BaseFile.spectra` interface.

Private helpers in this module build the output schema, pick which PSM to
emit when an .mszx scan has multiple, and flush per-row-group batches.
"""
...
def to_parquet(
    ...
    *,
    ...
    `from_parquet`: column names and dtypes match what the forward path
    consumes, including a `peptide_charge` column built from annotation
    peptide + charge so the forward path can re-split it.
    ...
    try:
        ...
        cols: Dict[str, List] = {name: [] for name in col_names}
        n_buffered = 0
