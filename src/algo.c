...
void
algo_decode_delta_transform_32f (void* args)
{
    // Parse args
    algo_args* a_args = (algo_args*)args;
    ...
    res = malloc((len + 1) * sizeof(uint16_t) + sizeof(float)); // Allocate space for result and leave room for header and first value
    ...
    *(float*)&res[0] = f[0];
    ...
    // Return result
    *a_args->dest = res;
    *a_args->dest_len = (len + 1) * sizeof(uint16_t) + sizeof(float);
 ...
 */
{
    ...
    #ifdef ERROR_CHECK
        if (a_args->src_format != _32f_) // non-essential check, but useful for debugging
            error("algo_encode_cast32: Unknown data format");
    #endif
...
void
algo_encode_delta_transform_32f (void* args)
{
    // Parse args
    algo_args* a_args = (algo_args*)args;
    ...
    // Get starting value
    float start = *(float*)((void*)(*a_args->src) + sizeof(u_int16_t));

    // Get source array
    uint16_t* arr = (uint16_t*)((void*)(*a_args->src) + sizeof(u_int16_t) + sizeof(float));
...
set_compress_algo(int algo, int accession)
{
    switch(algo)
    {
        ...
        {   
            switch(accession)
            {
                case _64d_ :    return algo_decode_cast32;
                case _32f_ :    return algo_decode_lossless; // casting 32 to 32 is just lossless
            }
        } ;
        case _delta_transform_ :
        {
            switch(accession)
            {
                case _32f_ :    return algo_decode_delta_transform_32f;
                case _64d_ :    
                {
                                //return algo_decode_delta_transform_64d;
...
{   
    switch(algo)
    {
        ...
        case _cast_64_to_32_:   
        {
            switch(accession)
            {
                case _64d_ :    return algo_encode_cast32;
        ...
        case _delta_transform_ :
        {
            switch(accession)
            {
                case _32f_ :    return algo_encode_delta_transform_32f;
                case _64d_ :    
                {
                                //return algo_encode_delta_transform_64d;
                                assert(0); // TODO: not implemented yet
                }
            }
        } ;
