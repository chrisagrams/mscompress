...
algo_decode_vbr_64d (void* args)
{
    ...
    uint16_t res_len = ((int)ceil(len*num_bits/8)) + sizeof(uint16_t) + sizeof(double);

    res = malloc(res_len); // Allocate space for result and leave room for header
    ...
    unsigned char* tmp_res = res + sizeof(uint16_t) + sizeof(double); // Ignore header
    ...
    for (int i = 0; i < len; i++) {
        if(tmp_index == sizeof(double)){
            ...
            uint64_t float_int = (uint64_t)floor(float64/base_peak_intensity * (exp2(num_bits)-1));

            for(int j = 0; j < num_bits; j++) {
                int bit = (float_int >> j) & 1;
                if(bit) tmp_res[b/8] |= 1 << (b%8);
                else    tmp_res[b/8] &= ~(1 << (b%8));
                b++;
            }
            result_index++;
            tmp_index = 0;
        }
        tmp_buff[tmp_index] = f[i];
        tmp_index++;
    }
    if(tmp_index == sizeof(double)){
        double float64;
    ...
    bytes_used = ceil((double)b/8);

    int padding = bytes_used*8 - b;

    for(int i = b; b < padding; b++) {
        tmp_res[b/8] &= ~(1 << (b%8));
    }
    ...
    // Return result
    *a_args->dest = res;
    *a_args->dest_len = sizeof(uint16_t) + sizeof(double) + res_len;
 ...
 */
{
    ...
    unsigned char* tmp_arr = arr + sizeof(uint16_t) + sizeof(double);
    ...
    // Move src pointer
    *a_args->src += sizeof(uint16_t) + sizeof(double) + (int)ceil(len*num_bits/8);
...
set_compress_algo(int algo, int accession)
{
    switch(algo)
    {
        ...
        case _vbr_ :
        {
            switch(accession)
            {
                case _32f_ :    assert(0); // not implemented yet
