...
#include "mscompress.h"
...
#ifdef _WIN32
  /*
   * Windows SDK >= 10.0.26100 expects one of the target-architecture
   * macros (_AMD64_, _X86_, _ARM_, _ARM64_, etc.) to be defined by the
   * compiler or prior headers. When building via Python/setuptools, the
   * MSVC predefined macros like _M_X64/_M_IX86/_M_ARM64 are present, but
   * the corresponding Windows SDK macros may not yet be set when including
   * <Windows.h>. Map the MSVC macros to the SDK architecture macros to
   * avoid winnt.h triggering "No Target Architecture".
   */
  ...
  /* Target a reasonably recent Windows version (Vista+) if not set */
  #ifndef _WIN32_WINNT
